...
tf.app.flags.DEFINE_string('output_directory', TRAIN_TF_RECORDS_DIR,
                           'Output data directory')

# B: he had different parameters here.
tf.app.flags.DEFINE_integer('train_shards', N_TRAIN_SHARDS,
                            'Number of shards in training TFRecord files.')
tf.app.flags.DEFINE_integer('validation_shards', N_VAL_SHARDS,
                            'Number of shards in validation TFRecord files.')
tf.app.flags.DEFINE_integer('num_threads', 1,#N_NUM_THREADS,
                            'Number of threads to preprocess the images.')
tf.app.flags.DEFINE_integer('num_train_threads', 1,# N_NUM_THREADS,  # needed to add this, so what does ^ do?
                            'Number of threads to preprocess the images.')
tf.app.flags.DEFINE_integer('num_val_threads', 1,# N_NUM_THREADS,
                            'Number of threads to preprocess the images.')

tf.app.flags.DEFINE_boolean('augmentation', False,  # B: we never do so this looks fine
...
def _process_image(filename, coder):
    ...
    Args:
      filename: string, path to an image file e.g., '/path/to/example.JPG'.
      coder: instance of ImageCoder to provide tf image coding
    Returns:
      image_buffer: string, JPEG encoding of RGB image.
      height: integer, image height in pixels.
      width: integer, image width in pixels.
...
def _process_image_files_batch(coder, thread_index, ranges, name, file_names, labels, num_shards):
    ...
    Args:
      coder: instance of ImageCoder to provide tf image coding
      thread_index: integer, unique batch to run index is within [0, len(ranges)).rf
...
def main(unused_argv):
    ...
    # Run it!
    _process_dataset(PREFIX_SHARD_VALIDATION, PATCHES_VALIDATION_DIR,
                     FLAGS.validation_shards, FLAGS.num_val_threads)
    _process_dataset(PREFIX_SHARD_TRAIN, PATCHES_TRAIN_DIR,
                     FLAGS.train_shards, FLAGS.num_train_threads)
