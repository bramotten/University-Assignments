...
import os, sys
x = os.path.dirname(os.path.dirname(os.path.realpath(__file__)))
x = x[:-1]
sep = '/'
x = x[:x.rfind(sep)] + sep
sys.path.append(x)
...
class PatchExtractor(object):
    ...
    def extract_positive_patches_from_tumor_region(wsi_image, tumor_gt_mask, level_used,
        ...
        for bounding_box in bounding_boxes:
            ...
            X = np.random.random_integers(
                b_x_start, high=b_x_end - 1, size=NUM_POSITIVE_PATCHES_FROM_EACH_BBOX)
            Y = np.random.random_integers(
                b_y_start, high=b_y_end - 1, size=NUM_POSITIVE_PATCHES_FROM_EACH_BBOX)

            for x, y in zip(X, Y):
                if int(tumor_gt_mask[y, x]) is PIXEL_WHITE:
                    patch = wsi_image.read_region((x * mag_factor, y * mag_factor), 0,
                                                  (PATCH_SIZE, PATCH_SIZE))
                    patch.save(patch_save_dir + patch_prefix +
                               str(patch_index)+ '.png', 'PNG')
                    patch_index += 1
                    patch.close()
    ...
    def extract_negative_patches_from_normal_wsi(wsi_image, image_open, level_used,
        ...
        for bounding_box in bounding_boxes:
            ...
            b_y_end = int(bounding_box[1]) + int(bounding_box[3])
            X = np.random.random_integers(
                b_x_start, high=b_x_end - 1, size=NUM_NEGATIVE_PATCHES_FROM_EACH_BBOX)
            Y = np.random.random_integers(
                b_y_start, high=b_y_end - 1, size=NUM_NEGATIVE_PATCHES_FROM_EACH_BBOX)

            for x, y in zip(X, Y):
                if int(image_open[y, x]) is not PIXEL_BLACK:
                    patch = wsi_image.read_region((x * mag_factor, y * mag_factor), 0,
                                                  (PATCH_SIZE, PATCH_SIZE))
                    patch.save(patch_save_dir + patch_prefix +
                               str(patch_index)+ '.png', 'PNG')
    ...
    def extract_negative_patches_from_tumor_wsi(wsi_image, tumor_gt_mask, image_open, level_used,
        ...
        for bounding_box in bounding_boxes:
            ...
            for x, y in zip(X, Y):
                # if len(image_open[0]) < 190:
                #     print(len(image_open[0]))
                if int(image_open[y, x]) is not PIXEL_BLACK and int(tumor_gt_mask[y, x]) is not PIXEL_WHITE:
        ...
        for bounding_box in bounding_boxes:
            ...
            for row in row_cords:
                for col in col_cords:
                    # consider pixels from ROI only
                    if int(image_open[row, col]) is not PIXEL_BLACK:
                        # extract patch corresponds to false positives
                        if heatmap_prob[row, col] >= TUMOR_PROB_THRESHOLD:
                            if int(tumor_gt_mask[row, col]) == PIXEL_BLACK:
                                # mask_gt does not contain tumor area
                                mask = wsi_mask.read_region((col * mag_factor, row * mag_factor), 0,
                                                            (PATCH_SIZE, PATCH_SIZE))
                                mask_gt = cv2.cvtColor(
                                    np.array(mask), cv2.COLOR_BGR2GRAY)
                                ...
                                if white_pixel_cnt_gt == 0:
                                    patch = wsi_image.read_region((col * mag_factor, row * mag_factor), 0,
                                                                  (PATCH_SIZE, PATCH_SIZE))
                                    patch.save(
                                        patch_save_dir_neg + patch_prefix_neg + str(patch_index)+ '.png', 'PNG')
                                    patch_index += 1
                                    patch.close()

                                mask.close()
                        # extract patch corresponds to false negatives
                        elif int(tumor_gt_mask[row, col]) is not PIXEL_BLACK \
                                and heatmap_prob[row, col] < TUMOR_PROB_THRESHOLD:
                            # mask_gt does not contain tumor area
                            mask = wsi_mask.read_region((col * mag_factor, row * mag_factor), 0,
                                                        (PATCH_SIZE, PATCH_SIZE))
                            mask_gt = cv2.cvtColor(
                            ...
                            if white_pixel_cnt_gt >= ((PATCH_SIZE * PATCH_SIZE) * 0.85):
                                patch = wsi_image.read_region((col * mag_factor, row * mag_factor), 0,
                                                              (PATCH_SIZE, PATCH_SIZE))
                                patch.save(
                                    patch_save_dir_pos + patch_prefix_pos + str(patch_index)+ '.png', 'PNG')
                                patch_index += 1
                                patch.close()
                ...
                for col in col_cords:
                    ...
                    if int(image_open[row, col]) is not PIXEL_BLACK:
                        ...
                        if heatmap_prob[row, col] >= TUMOR_PROB_THRESHOLD:
                            ...
                            patch = wsi_image.read_region((col * mag_factor, row * mag_factor), 0,
                                                          (PATCH_SIZE, PATCH_SIZE))
                            patch.save(patch_save_dir_neg +
                                       patch_prefix_neg + str(patch_index)+ '.png', 'PNG')
                            patch_index += 1
                            patch.close()
...
class WSIOps(object):
    ...
    def read_wsi_tumor(wsi_path, mask_path):
        ...
        try:
            ...
            tumor_gt_mask = cv2.resize(
                np.array(tumor_gt_mask), (0, 0), fx=resize_factor, fy=resize_factor)
    ...
    def find_roi_bbox(self, rgb_image):
        ...
        bounding_boxes, rgb_contour = self.get_bbox(
            image_open, rgb_image=rgb_image)
    ...
    def get_bbox(cont_img, rgb_image=[None]):
        # Default rgb_image used to be None. The np.any() was missing too.
        _, contours, _ = cv2.findContours(
            cont_img, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        rgb_contour = None
        if np.any(rgb_image) != None:
