...
tf.flags.DEFINE_boolean('mkl', False, 'If true, set MKL environment variables.')
...
NUM_TRAIN_IMAGES = 77871
NUM_EVAL_IMAGES = 8653
...
def gradual_warmup_then_dec(learning_rate, warmup_steps, end_learning_rate, global_step, steps, name=None):
  with ops.name_scope(name, "gradual_warmup_then_dec",
    ...
    _3ep = int(240) #int(steps/4)
    _12ep = int(940)
    _30ep = int(2340)
    _5ep = int(390)
    #steps_decay2 = #int(steps/8)
    #steps_warmup = #int(steps/30)
    #decay_rate = tf.cast(decay_rate, dtype)
    #value_size = len(steps)+1
    #steps = ops.convert_n_to_tensor(steps)
    #values = [tf.multiply(end_learning_rate, tf.pow(decay_rate, i)) for i in range(value_size)]i
    ...
    #                    #lambda: tf.train.cosine_decay_restarts(end_learning_rate, global_step-warmup_steps ,2400, t_mul=0.5, m_mul=0.5, alpha=0.0))
    #lr = tf.case([(tf.less(global_step, warmup_steps), lambda: warmup_decay(learning_rate, global_step, warmup_steps, end_learning_rate)),
    #              (tf.less(global_step, warmup_steps+_30ep), lambda: tf.train.polynomial_decay(4.8, global_step-warmup_steps, _30ep, 3.36, power=1)),
    #              (tf.less(global_step, warmup_steps+_30ep+_12ep), lambda: tf.train.polynomial_decay(3.36, global_step-warmup_steps-_30ep, _12ep, 0.84, power=2)),
    #              (tf.less(global_step, warmup_steps+_30ep+_12ep+_3ep), lambda: warmup_decay(0.84, global_step-warmup_steps-_30ep-_12ep, _3ep, 2.52)),
    #              (tf.less(global_step, warmup_steps+_30ep+_12ep+_3ep+_12ep), lambda: tf.train.polynomial_decay(2.52, global_step-warmup_steps-_30ep-_12ep-_3ep, _12ep, 0.63, power=2)),
    #              (tf.less(global_step, warmup_steps+_30ep+_12ep+_3ep+_12ep+_3ep), lambda: warmup_decay(0.63, global_step-warmup_steps-_30ep-_12ep-_3ep-_12ep, _3ep, 1.89)),
    #              (tf.less(global_step, warmup_steps+_30ep+_12ep+_3ep+_12ep+_3ep+_12ep), lambda: tf.train.polynomial_decay(1.89, global_step-warmup_steps-_30ep-_12ep-_3ep-_12ep-_3ep, _12ep, 0.47, power=2)),
    #              (tf.less(global_step, warmup_steps+_30ep+_12ep+_3ep+_12ep+_3ep+_12ep+_3ep), lambda: warmup_decay(0.47, global_step-warmup_steps-_30ep-_12ep-_3ep-_12ep-_3ep-_12ep, _3ep, 1.41)),                  
    #              (tf.less(global_step, warmup_steps+_30ep+_12ep+_3ep+_12ep+_3ep+_12ep+_3ep+_12ep), lambda: tf.train.polynomial_decay(1.41, global_step-warmup_steps-_30ep-_12ep-_3ep-_12ep-_3ep-_12ep-_3ep, _12ep, 0.47, power=2)),
    #              (tf.less(global_step, warmup_steps+_30ep+_12ep+_3ep+_12ep+_3ep+_12ep+_3ep+_12ep+_5ep), lambda: tf.train.polynomial_decay(0.47, global_step-warmup_steps-_30ep-_12ep-_3ep-_12ep-_3ep-_12ep-_3ep-_12ep, _5ep, 0.0, power=2))])
#                  (tf.less(global_step, warmup_steps), lambda: warmup_decay(end_learning_rate/4, global_step, steps_warmup, end_learning_rate/2)),
#                  (tf.less(global_step, warmup+decay_step), lambda: tf.train.polynomial_decay(end_learning_rate, global_step-warmup_steps, steps-warmup_steps, end_learning_rate, power=1)),
#		  (tf.less(global_step, warmup_steps), lambda: warmup_decay(learning_rate, global_step, warmup_steps, end_learning_rate)),
#],
                        #lambda: lr_decrease(start_lr, end_lr, num_steps)
                        #lambda: tf.train.polynomial_decay(end_learning_rate, global_step-warmup_steps, steps-warmup_steps, 0, power=1))
                        #lambda: tf.train.cosine_decay_restarts(end_learning_rate, global_step-warmup_steps ,2400, t_mul=0.5, m_mul=0.5, alpha=0.0))
    return lr
...
def resnet_model_fn(features, labels, mode, params):
  ...
  #tf.train.init_from_checkpoint(FLAGS.checkpoint_path, {v.name.split(':')[0]: v for v in tf.contrib.framework.get_variables_to_restore(exclude=exclude)})#variables_to_restore})
  #print(labels.shape)
  ...
  if mode == tf.estimator.ModeKeys.TRAIN:
    ...
    learning_rate = gradual_warmup_then_dec(0.1, int(batches_per_epoch * 5) , 0.8, global_step, FLAGS.train_steps, name="gradual_warmup_then_dec")
    learning_rate_previous = gradual_warmup_then_dec(0.1, int(batches_per_epoch * 5) , 0.8,  global_step-1, FLAGS.train_steps, name="gradual_warmup_then_dec")
    #momentum_corr = tf.cond(tf.less(global_step, int(batches_per_epoch * 5)),
    #                    lambda: learning_rate/learning_rate_previous, #warmup_decay(learning_rate, global_step, warmup_steps, end_learning_rate),
    #                    lambda: 1.0) #tf.train.polynomial_decay(end_learning_rate, global_step-warmup_steps, steps-warmup_steps, 0, power=1))
  ...
  #create a tensor for logging accuracy
...
def main(unused_argv):
  ...
  config = tpu_config.RunConfig(
      master=tpu_grpc_url,
      save_checkpoints_secs=600 if hvd.rank() ==0 else 0,
