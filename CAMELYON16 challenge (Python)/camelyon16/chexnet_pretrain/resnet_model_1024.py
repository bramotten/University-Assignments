...
def resnet_v1_wide_generator(block_fn, layers, num_classes,
                        data_format='channels_first'):
...
def resnet_v1_generator(block_fn, layers, num_classes,
  ...
  def model(inputs, is_training):
    ...
    inputs = block_group(
        ...
        data_format=data_format)
    inputs = block_group(
        inputs=inputs, filters=1024, block_fn=block_fn, blocks=layers[4],
        strides=2, is_training=is_training, name='block_group5',
        data_format=data_format)
    inputs = block_group(
        inputs=inputs, filters=2048, block_fn=block_fn, blocks=layers[5],
        strides=2, is_training=is_training, name='block_group5',
        data_format=data_format)
    ...
    inputs = tf.identity(inputs, 'final_avg_pool')
    inputs = tf.reshape(
        inputs, [-1, 8192 if block_fn is bottleneck_block else 512])
    inputs = tf.layers.dense(
        inputs=inputs,
        units=num_classes,
        kernel_initializer=tf.random_normal_initializer(stddev=.01))
...
def resnet_v1(resnet_depth, num_classes, data_format='channels_first'):
  ...
  model_params = {
      ...
      70: {'block': bottleneck_block, 'layers': [3, 3, 4, 6, 3]},
      72: {'block': bottleneck_block, 'layers': [2, 3, 4, 6, 4, 3]},
